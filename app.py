"""
kath.fund TM-2Klassen — Fundbüro · Katharineum zu Lübeck
Modell: Teachable-Machine tm-my-image-model (Taschenrechner vs. Federtasche),
konvertiert nach tm-model.onnx. Original: tm-my-image-model.zip -> model/.
Altes Repo: Liamlol1919/Fundb-roprototypkochjugit (ImageNet-ONNX) — hier ersetzt durch TM-Modell.
---
...
CATEGORIES = [
    "Taschenrechner", "Federtasche", "Sonstiges",
]
# Hinweis: Dieses Repo erkennt NUR Taschenrechner vs. Federtasche
# (Teachable-Machine-Modell tm-my-image-model). Alles unter 50 % -> Sonstiges.
LOCATIONS = ["Hauptgebäude - Foyer", "Pausenhof", "Sporthalle", "Mensa / Cafeteria",
             "Bibliothek", "Fachräume / MINT", "Musiksaal", "Unbekannt"]

KAT_ICON = {
    "Taschenrechner": "🔢", "Federtasche": "✏️", "Sonstiges": "📦",
}

DEFAULT_ITEMS = [
    {"id": 1001, "titel": "Taschenrechner Casio fx-991DE", "kategorie": "Taschenrechner",
     "fundort": "Fachräume / MINT", "abgabeort": "Hausmeisterbüro (Raum 001)",
     "datum_fund": "2026-09-01", "datum_ablauf": "2026-12-01", "status": "Offen",
     "beschreibung": "Schwarz, Display intakt, Name auf Rückseite abgeklebt.",
     "image_file": None, "tags": ["Casio", "Rechner", "MINT"]},
    {"id": 1002, "titel": "Blaue Federtasche mit Filzstiften", "kategorie": "Federtasche",
     "fundort": "Bibliothek", "abgabeort": "Sekretariat (Tresor)",
     "datum_fund": "2026-09-05", "datum_ablauf": "2026-12-05", "status": "Offen",
     "beschreibung": "Rot kariert, ca. 30 Filzstifte, Initialen „J.K.“.",
...
# =============================================================================
# =============================================================================
# 3. KI (Teachable-Machine tm-my-image-model: Taschenrechner vs. Federtasche)
# Quelle: tm-my-image-model.zip -> model/model.json + model/weights.bin +
# model/metadata.json, konvertiert nach tm-model.onnx (s. tools/convert_tm.py).
# Preprocessing exakt wie Teachable Machine: 224x224, float32, (px/127.5)-1, NHWC.
# Nur 2 Klassen — unter 50 % Konfidenz -> Sonstiges.
# =============================================================================

TM_LABELS = ["Taschenrechner", "Federtasche"]
TM_ONNX = Path("tm-model.onnx")


@st.cache_resource(show_spinner="🧠 TM-Modell wird geladen — bitte ca. 10 Sekunden warten …")
def load_vision_model():
    try:
        import onnxruntime as ort
        if TM_ONNX.exists():
            session = ort.InferenceSession(str(TM_ONNX), providers=["CPUExecutionProvider"])
            return session, TM_LABELS, "tm"
        return None
    except Exception:
        return None
...
def analyze_image_ai(pil_image):
    ...
    if vision is not None:
        try:
            ...
            conf = float(probs[j])
            label = labels[j]
            engine = f"TM-onnx · {label}"
            if conf < 0.5:
                return "Sonstiges", conf, engine + " · unsicher"
            return label, conf, engine
        except Exception:
            pass
    ...
    if std < 12:
        return "Sonstiges", 0.35, "Bildmerkmale · unsicher"
    if w / float(h) < 0.62 or w / float(h) > 1.7:
        return "Federtasche", 0.45, "Bildmerkmale · unsicher"
...
const KAT_ICON = {
  "Taschenrechner": '<rect x="4" y="2" width="16" height="20" rx="2"/><path d="M8 6h8"/><path d="M8 11h.01M12 11h.01M16 11h.01M8 15h.01M12 15h.01M16 15h.01M8 19h.01M12 19h.01M16 19h.01"/>',
  "Federtasche": '<path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/>',
